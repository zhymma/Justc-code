...
class MwpBertModel_CLS_classfier(torch.nn.Module):
    def __init__(self, bert_path_or_config, num_labels, train_loss='MSE', fc_path=None, multi_fc=False,
        ...
        self.t5.load_t5(t5_weight.state_dict())
        self.tokenizer = T5Tokenizer.from_pretrained('t5-large')

        if multi_fc:
            #! 768*2 -> 2048 -> 1024 -> 28
            self.fc = Batch_Net_CLS(d_model * 2, fc_hidden_size, int(fc_hidden_size / 2),
                                    num_labels)
        

        # self.get_goal1 = nn.Linear(d_model * 2, fc_hidden_size)
        # self.get_goal2 = nn.Linear(d_model * 2, fc_hidden_size)
        # self.get_goal3 = nn.Linear(fc_hidden_size,d_model)
        # self.get_goal4 = nn.Linear(fc_hidden_size,d_model)
